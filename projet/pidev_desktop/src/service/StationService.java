...
public class StationService implements IService<Station> {
    ...
    Connection cnx = DataSource.getInstance().getCnx();
     ...
     public void ajouter(Station s) {
        try {
            String req = "INSERT INTO `station` (`long_alt`, `ligne_id`) VALUES (?, ?)";
            PreparedStatement ps = cnx.prepareStatement(req);
            ps.setString(1, s.getLang_alt());
            ps.setInt(2, s.getLigne_id()); // Ajoutez le champ ligne_id ici
    ...
    public void modifier(Station s) {
        try {
            String req = "UPDATE `station` SET `long_alt`=?, `ligne_id`=? WHERE id=?";
            PreparedStatement ps = cnx.prepareStatement(req);
            ps.setString(1, s.getLang_alt());
            ps.setInt(2, s.getLigne_id()); // Ajoutez le champ ligne_id ici
     ...
     public List<Station> getAll() {
        ...
        try {
            ...
            while (rs.next()) {
                Station s = new Station(rs.getInt("id"), rs.getString("long_alt"), rs.getInt("ligne_id"));
                list.add(s);
            }
        } catch (SQLException ex) {
            System.out.println(ex.getMessage());
...
public void insert(Station t) {
    try {
        String req = "INSERT INTO `station` (`long_alt`, `ligne_id`) VALUES (?, ?)";
        PreparedStatement ps = cnx.prepareStatement(req);
        ps.setString(1, t.getLang_alt());
        ps.setInt(2, t.getLigne_id());
