    ...
    public function newReservation(Request $request, ReservationRepository $reservationRepository,MailerInterface $mailer,AuthenticationUtils $authenticationUtils,UserRepository $userRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $email = (new TemplatedEmail())
                ->from(Address::create('Swift Transit <swiftTransitOriginal1@hotmail.com>'))
                ->to($user->getEmail())
 ...
 */
    ...
    #[Route('/trajet', name: 'voyager_trajet', methods: ['GET'])]
    public function listTrajet(LigneRepository $ligneRepository): Response
    ...
    #[Route('/trajet/{id}', name: 'app_client_ligne_show', methods: ['GET'])]
    public function listStation(Ligne $ligne, StationRepository $stationRepository): Response
    {
       $stations = $stationRepository->findBy(['ligne' => $ligne]);

       return $this->render('trajet/front_ligne_show.html.twig', [
            'ligne' => $ligne,
            'stations' => $stations,
        ]);
    }
    #[Route('/trajet/{id}/pdf', name: 'app_client_ligne_pdf', methods: ['GET'])]
    public function generatePdf(Ligne $ligne, StationRepository $stationRepository, \Knp\Snappy\Pdf $pdf)
{
    $stations = $stationRepository->findBy(['ligne' => $ligne]);
...
public function listTrajets(StationRepository $stationRepository): Response
{
    ...
    foreach ($stations as $station) {
        $longitudeAltitude = $station->getLongAlt();
        
        if ($longitudeAltitude) {
            // Split the string into latitude and longitude based on the separator (e.g., '/')
            list($longitude, $altitude) = explode('/', $longitudeAltitude);
